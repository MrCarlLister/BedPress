 ...
 * @return void
 */
function ee_mph__acf_register_longform_media_block()
{

    if (!function_exists('acf_register_block_type'))
        return;

    $block_name = 'longform-media';
    $render_template = ee_mph__acf_find_the_template_path($block_name);

    acf_register_block_type(array(
        'name'                => $block_name,
        'title'                => __('Longform with media', get_bloginfo('name')),
        'description'        => __('Display a long form element - for article writing - with option media element'),
        'category'            => 'common',
        'icon'                => 'media-document',
        'keywords'            => array('quote', 'image', 'text'),
...
add_action('acf/init', 'ee_mph__acf_register_longform_media_block');
